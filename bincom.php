...
public function getpollingUnit(){
 //write query to fetch a polling unit
	...
	$sql="SELECT * FROM polling_unit WHERE polling_unit_id=1 ";

		//check if the query() runs the sql statement//to fetch
   
   if($result=$this->dbcon->query("SELECT * FROM polling_unit WHERE polling_unit_id=1 ")){
...
//write query to fetch all party

	$sql="SELECT * FROM announced_pu_results";

		//check if the query() runs the sql statement//to fetch
   
   if($result=$this->dbcon->query("SELECT * FROM announced_pu_results ")){
